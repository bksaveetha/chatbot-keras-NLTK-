...
def clean_up_sentence(sentence):
    ...
    # stem each word- create short fprm for word
...
def bow(sentence, words, show_details = True):
    ...
    for s in sentence_words:
        for i, w in enumerate(words):
            if w == s:
                # assign 1 if current word in the vocabulary position
                bag[i] = 1
                if show_details:
...
def predict_class(sentence, model):
    ...
    # sort by strenght of probability
...
#create button  to enter the message
